...
\begin{frame}{3.1 Variables \& Assumed Constants}
    \begin{table}
        ...
        \begin{tabular}{l p{5.2cm} l l}
            ...
            $N$ & Number of attendees & Persons & Parameter \\
            $S$ & Number of food stalls & Stalls & \textbf{Decision Var} \\
            $A$ & Activity duration (concerts, events) & Hours & Decision Var \\
            ...
            $\beta_N$ & Crowding exponent (Urban scaling) & 1.3 & Constant \\
            $\gamma_{NS}$ & Congestion penalty coefficient & 0.0005 & Constant \\
            \bottomrule
        \end{tabular}
    \end{table}

    \footnotesize \textbf{Ref:} Bettencourt et al.\ (2007) for $\beta_N=1.3$; CPCB (2021) for waste norms; CEA (2024) for grid emission.
\end{frame}

% --- Slide 3/4 ---
\begin{frame}{3.1 Mathematical Formulation}
    \begin{alertblock}{Environmental Load Function}
        $$ E = \underbrace{(\alpha_1 N + \alpha_2 S)}_{\text{Base Load}} + \underbrace{(\beta_N N^{1.3})}_{\substack{\text{Non-linear}\\\text{Crowding}}} + \underbrace{\left( \gamma_{NS} \frac{N^2}{S} \right)}_{\text{Congestion Penalty}} $$
    \end{alertblock}

    \textbf{Component Interpretation:}
    \begin{itemize}
        \item \textbf{Base Load} ($\alpha_1 N + \alpha_2 S$): Direct consumption proportional to attendees and infrastructure.
        \item \textbf{Non-linear Crowding} ($N^{1.3}$): Super-linear waste generation --- more people $\Rightarrow$ disproportionately more littering (Urban Scaling Law).
...
\begin{frame}{3.1 Optimization \& Insights}
    \begin{exampleblock}{Unconstrained Optimum}
        Differentiating $E$ w.r.t.\ $S$ and setting to zero:
        $$ \frac{dE}{dS} = \alpha_2 - \gamma_{NS} \frac{N^2}{S^2} = 0 \implies \boxed{S^* = N \sqrt{\frac{\gamma_{NS}}{\alpha_2}}} $$
    \end{exampleblock}

    \begin{columns}[T]
        \begin{column}{0.48\textwidth}
            \textbf{Numerical Result ($N = 40{,}000$/day):}
            $$ S^* = 40000 \sqrt{\frac{0.0005}{18}} \approx \mathbf{210} $$
        \end{column}
        \begin{column}{0.48\textwidth}
            \textbf{Key Insights:}
            \begin{itemize}
...
\begin{frame}{3.3 Variables, Constants \& Data}
    \begin{table}
        ...
        \begin{tabular}{l p{4.8cm} l l}
            ...
            \midrule
            $V_{cap}$ & Vehicle payload (Tata Ace EV) & 750 kg & Constant \\
            $C_{dist}$ & Transport cost per km & Rs.\,20/km & Constant \\
            $C_{handling}$ & Handling cost per kg & Rs./kg & Constant \\
            $C_{fixed}$ & Fixed cost per vehicle per day & Rs.\,800 & Constant \\
